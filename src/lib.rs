...
macro_rules! plugin {
    ($name:ident) => {
        ...
        fn __new_plug(_: &PluginDescriptor, sample_rate: u64) -> Box<dyn Plugin + Send> {
        ...
        impl Plugin for __plug {
            fn run<'a>(&mut self, _: usize, ports: &[&'a PortConnection<'a>]) {
                ...
                for i in 0..self.channels {
                    ...
                    output.push(unsafe {
                        mem::transmute(
                            &ports[i + self.channels].unwrap_audio_mut()[..] as *const [f32],
                        )
                    });
                }
                ...
                if bins != self.bins || time_div != self.time_div {
                    ...
                    self.plugin =
                        $name::new(self.channels, self.sample_rate, self.bins, self.time_div);
                }
                let sample_rate = self.sample_rate;
                let plugin = &mut self.plugin;
                let user_ports = ports[2..]
                    .iter()
                    .map(|&x| *x.unwrap_control() as f64)
                    .collect::<Vec<f64>>();
                self.pvoc.process(
                    &input,
                    &mut output,
